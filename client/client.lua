...
exports('identification', function(data, slot)
	if not LocalPlayer.state.idshown  then 
		ox_inventory:useItem(data, function(data)
			if data then
				TriggerEvent('sho_identification:showID',data)
			end
		end)
	else
...
RegisterNetEvent('sho_identification:showID')
AddEventHandler('sho_identification:showID', function(item)
	...
	if not LocalPlayer.state.idshown  then 
		...
		if item.name == 'police_license' then
			ExecuteCommand('e idcardh')
			showdage = true
		else
			ExecuteCommand('e idcardc')
		end
		if #playersInArea > 0 then 
			...
			TriggerServerEvent('sho_identification:server:showID',item,Playerinareaid)
			TriggerEvent('sho_identification:openID',item)
...
-- Event to show your ID to nearby players
RegisterNetEvent('sho_identification:openID')
AddEventHandler('sho_identification:openID', function(item)
	--print("opening ID")
	--print(LocalPlayer.state.idvisible)
	if LocalPlayer.state.idvisible == nil or not LocalPlayer.state.idvisible then 
		TriggerEvent('sho_identification:showUI',item)
	end 
end)

-- NUI Events 
-- We define a "stop" command inside this too
RegisterNetEvent('sho_identification:showUI')
AddEventHandler('sho_identification:showUI', function(data)
...
RegisterCommand('issueidcard', function()
	exports['mugshot']:getMugshotUrl(ESX.PlayerData.ped, function(url)
		TriggerServerEvent('sho_identification:createCard',source,url,"identification")
	end)
end, false)

RegisterCommand('issuedriverslicense', function()
	exports['mugshot']:getMugshotUrl(ESX.PlayerData.ped, function(url)
		TriggerServerEvent('sho_identification:createCard',source,url,"drivers_license")
	end)
end, false)

RegisterCommand('issuefirearmslicense', function()
	exports['mugshot']:getMugshotUrl(ESX.PlayerData.ped, function(url)
		TriggerServerEvent('sho_identification:createCard',source,url,"firearms_license")
	end)
end, false)
]]--
